...
plt.plot(df1["Date"], df1["Close"], 'r-', linewidth=0.6, label="Adidas Stock, mean="+str(mean))
# or the same can be:
...
plt.figure("Puma Stock")
plt.plot(df2["Date"], df2["Close"], 'r-', linewidth=0.6, label="Puma stock, mean="+str(mean2))
# or the same can be:
# plt.plot("Date", "Close", 'r-', linewidth=0.6, label="APPL Stock price, mean="+str(mean), data=df1)
